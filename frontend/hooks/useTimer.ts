...
export function useTimer(): TimerState {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (remain === 0 && !alarmFiredRef.current) {
        ...
        alarmDismissRef.current = setTimeout(() => setAlarmVisible(false), 12000);
      }
    };
  ...
  function startTimer(label: string, minutes: number) {
    ...
    // 서비스 워커 백그라운드 타이머 동기화
    if (typeof window !== 'undefined' && 'serviceWorker' in navigator) {
      navigator.serviceWorker.ready.then((registration) => {
        if (registration.active) {
          registration.active.postMessage({ type: 'START_TIMER', label, endMs });
        }
      }).catch(() => {});
    }

    // Web Audio Context 초기화 (사용자 제스처 타이밍에 언락)
    try {

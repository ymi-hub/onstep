...
function HubView({ onOpenSessions, onOpenTracker, onOpenCare, onOpenMedication, onOpenHealth, onOpenDiet }: {
  ...
  return (
    <div style={{ background: '#FAFAF8', minHeight: '100%' }}>
      ...
      {/* 시작 가이드 배너 */}
      <div style={{ margin: '16px 24px 0', padding: '14px 24px', background: '#0C0C0A', borderRadius: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ padding: '16px 24px 8px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, alignItems: 'start' }}>
  ...
  return (
    <div style={{ position: 'fixed', top: 0, bottom: 0, left: 'max(0px,calc(50vw - 215px))', right: 'max(0px,calc(50vw - 215px))', zIndex: 100, background: '#FAFAF8', display: 'flex', flexDirection: 'column', overflowY: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {loading ? (
          <div style={{ padding: '48px 24px', textAlign: 'center', color: '#9A9490', fontFamily: font, fontSize: 13 }}>로딩 중...</div>
        ) : (
          <div>
            {filteredSessions.length === 0 && search.trim() ? (
              <div style={{ padding: '36px 24px', textAlign: 'center', fontFamily: font, fontSize: 13, color: '#9A9490' }}>
            ...
            {filteredSessions.slice(0, visibleCount).map((s, idx) => {
              ...
              return (
                <div key={s.id}>
                  ...
                  {isExpanded && (
                    <div style={{ padding: '14px 24px 16px', background: '#F4F4F0', borderBottom: '1px solid rgba(12,12,10,.07)' }}>
...
function EditorView({
  ...
  return (
    <div style={{ position: 'fixed', top: 0, bottom: 0, left: 'max(0px,calc(50vw - 215px))', right: 'max(0px,calc(50vw - 215px))', zIndex: 200, background: '#FAFAF8', display: 'flex', flexDirection: 'column', overflowY: 'hidden' }}>
      ...
      {/* 하단 고정 저장 버튼 */}
      <div style={{ flexShrink: 0, padding: '10px 24px 16px', background: 'rgba(250,250,248,.97)', borderTop: '1px solid rgba(12,12,10,.07)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}>
          ...
          <div style={{ position: 'fixed', bottom: 0, left: 'max(0px,calc(50vw - 215px))', right: 'max(0px,calc(50vw - 215px))', zIndex: 310, background: '#FAFAF8', borderRadius: '20px 20px 0 0', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 -4px 40px rgba(0,0,0,.12)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {domainProducts.length === 0 && !pickerSearch.trim() ? (
                <div style={{ padding: '40px 24px', textAlign: 'center', color: '#9A9490', fontFamily: f, fontSize: 13, lineHeight: 1.6 }}>BOX에 뷰티 제품이 없습니다.<br />BOX 탭에서 먼저 제품을 추가해주세요.</div>
              ) : (
                <>
                  {filteredProducts.map((p) => {
                    const isSel = pickerSelected.has(p.id);
                    return (
                      <div key={p.id} onClick={() => setPickerSelected((prev) => { const n = new Set(prev); if (n.has(p.id)) n.delete(p.id); else n.add(p.id); return n; })} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 24px', borderBottom: '1px solid rgba(12,12,10,.07)', cursor: 'pointer', background: isSel ? 'rgba(197,255,0,.06)' : 'transparent' }}>
                  ...
                  {pickerSearch.trim() && filteredProducts.length === 0 && (
                    <div onClick={() => registerAndAdd(pickerSearch)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 24px', cursor: 'pointer', background: 'rgba(197,255,0,.06)', borderBottom: '1px solid rgba(12,12,10,.07)' }}>
            ...
            <div style={{ padding: '12px 24px calc(env(safe-area-inset-bottom, 0px) + 32px)', flexShrink: 0, borderTop: '1px solid rgba(12,12,10,.07)' }}>
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 'max(0px,calc(50vw - 215px))', right: 'max(0px,calc(50vw - 215px))', zIndex: 330, background: '#FAFAF8', borderRadius: '20px 20px 0 0', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 -4px 40px rgba(0,0,0,.15)' }}>
        ...
        {/* 헤더 */}
        <div style={{ padding: '12px 24px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(12,12,10,.07)', flexShrink: 0 }}>
          <div>
      ...
      <div style={{ position: 'fixed', inset: 0, background: '#FAFAF8', zIndex: 50, display: 'flex', flexDirection: 'column', maxWidth: 430, margin: '0 auto' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, paddingBottom: 24 }}>
            {!isNew && editProgram?.id && <button onClick={() => handleDelete(editProgram.id)} style={{ padding: '12px 24px', background: '#FEE2E2', border: 'none', borderRadius: 12, fontFamily: f, fontSize: 13, fontWeight: 700, color: '#DC2626', cursor: 'pointer' }}>삭제</button>}
        ...
        <>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#FAFAF8', borderRadius: '20px 20px 0 0', zIndex: 21, display: 'flex', flexDirection: 'column', maxHeight: '75vh' }}>
            ...
            {pickerStep === 'select' ? (
              <>
                {/* 헤더 — 선택 단계 */}
                <div style={{ padding: '16px 24px 10px', borderBottom: '1px solid rgba(12,12,10,.07)' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                  ...
                  {filteredPickerProds.length === 0 && !pickerSearch.trim() ? (
                    <div style={{ padding: '40px 24px', textAlign: 'center', color: '#9A9490', fontFamily: f, fontSize: 13, lineHeight: 1.6 }}>
                    ...
                    filteredPickerProds.map(p => {
                      ...
                      return (
                        <div key={p.id} onClick={() => setPickerSelected(prev => { const n = new Set(prev); if (n.has(p.id)) n.delete(p.id); else n.add(p.id); return n; })}
                          style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 24px', borderBottom: '1px solid rgba(12,12,10,.07)', cursor: 'pointer', background: isSel ? 'rgba(197,255,0,.06)' : 'transparent' }}>
                ...
                {/* 다음 버튼 */}
                <div style={{ padding: '12px 24px calc(env(safe-area-inset-bottom,0px) + 16px)', borderTop: '1px solid rgba(12,12,10,.07)' }}>
              ...
              <>
                {/* 헤더 — 수량 입력 단계 */}
                <div style={{ padding: '16px 24px 12px', borderBottom: '1px solid rgba(12,12,10,.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg,#fdf4ff,#e0a0ff)', borderRadius: 16, padding: '16px', marginBottom: 12, border: '1px solid rgba(124,58,237,.2)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ padding: '9px 24px', background: presetLoading ? '#9B7FD4' : '#7C3AED', border: 'none', borderRadius: 9, fontFamily: f, fontSize: 12, fontWeight: 800, color: '#fff', cursor: presetLoading ? 'default' : 'pointer', flexShrink: 0 }}>
...
function MedView({
  ...
  return (
    <div style={{ position: 'fixed', top: 0, bottom: 0, left: 'max(0px,calc(50vw - 215px))', right: 'max(0px,calc(50vw - 215px))', zIndex: 100, background: '#FAFAF8', display: 'flex', flexDirection: 'column', overflowY: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {/* Hero */}
        <div style={{ padding: '28px 24px 20px', borderBottom: '1px solid rgba(12,12,10,.07)', position: 'relative' }}>
        ...
        {(() => {
          ...
          const GroupedList = ({ meds }: { meds: MedRoutine[] }) => (
            <>
              {MED_GROUPS.map(g => {
                ...
                return (
                  <div key={g.key}>
                    <div style={{ padding: '8px 24px 4px', background: '#F8F8F6', borderTop: '1px solid rgba(12,12,10,.05)' }}>
          ...
          return (
            <div style={{ padding: '20px 24px 0' }}>
              ...
              {items.length === 0 ? (
                <div style={{ padding: '36px 24px', textAlign: 'center', fontFamily: f, fontSize: 13, color: '#9A9490', lineHeight: 1.6, border: '1.5px dashed rgba(12,12,10,.14)', borderRadius: 16, background: '#EEEDE9', marginTop: 8 }}>
                  아직 등록된 약이 없습니다.<br />위에서 새 약을 추가해주세요.
                </div>
              ) : medSearch.trim() ? (
                filteredMeds.length === 0 ? (
                  <div style={{ padding: '36px 24px', textAlign: 'center', fontFamily: f, fontSize: 13, color: '#9A9490', marginTop: 8 }}>
                ...
                <div style={{ padding: '20px 24px', textAlign: 'center', fontFamily: f, fontSize: 13, color: '#BCBAB6', marginTop: 8 }}>
                  모두 Today에 표시 중입니다.
                </div>
              ) : (
                <div style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.06),0 0 0 1px rgba(0,0,0,.04)', marginTop: 8 }}>
          ...
          return (
            <div style={{ padding: '24px 24px 0' }}>
              <div style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(12,12,10,.07)', boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
                {/* 카드 헤더 */}
                <div style={{ padding: '14px 24px 12px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                  <div>
      ...
      {showCatForm && (
        <>
          ...
          <div style={{ position: 'fixed', bottom: 0, left: 'max(0px,calc(50vw - 215px))', right: 'max(0px,calc(50vw - 215px))', zIndex: 311, background: '#FAFAF8', borderRadius: '20px 20px 0 0', padding: '10px 20px calc(env(safe-area-inset-bottom, 0px) + 28px)', maxHeight: '80%', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {catEditId && <button onClick={() => deleteCat(catEditId)} style={{ padding: '12px 24px', background: '#FEE2E2', border: 'none', borderRadius: 12, fontFamily: f, fontSize: 13, fontWeight: 700, color: '#DC2626', cursor: 'pointer' }}>삭제</button>}
  ...
  function CtCard({ item }: { item: CtItem }) {
    ...
    return (
      <div style={{ background: '#fff', border: `1.5px solid ${item.published ? '#0C0C0A' : 'rgba(12,12,10,.07)'}`, borderRadius: 16, overflow: 'hidden', marginBottom: 12, transition: 'border-color .2s' }}>
        ...
        <div style={{ borderTop: '1px solid rgba(12,12,10,.07)', padding: '10px 24px 12px', display: 'flex', gap: 8 }}>

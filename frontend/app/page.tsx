  ...
  return (
    <div style={{ margin: '0 20px 12px', background: '#FFFFFF', borderRadius: 20, boxShadow: '0 2px 10px rgba(0,0,0,.05)', border: '1px solid rgba(0,0,0,.04)', padding: '16px 20px 20px' }}>
      ...
      {(() => {
        if (!session) {
          return (
            <span style={{ fontSize: 12, fontWeight: 500, color: '#C7C7CC' }}>
        ...
        const todayW   = totalDays <= 7 ? 22 : totalDays <= 14 ? 18 : 12;
        return (
...
function FlowCard({
  ...
  return (
    ...
    >
      {/* ① 최상단: MORNING / NIGHT 탭 — 언더라인 스타일 */}
      <div style={{ display: 'flex', borderBottom: '1px solid rgba(0,0,0,.06)', padding: '16px 20px 0' }}>
        ...
        <div style={{ padding: '10px 20px 0' }}>
          ...
          {slot.expertTip && (
            <div style={{ position: 'relative', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', padding: 24, gap: 8, background: '#FAFAFA', border: '1px solid #E4E4E7', borderRadius: 16, marginTop: 8, marginBottom: 12, overflow: 'visible' }}>
              ...
              <div style={{ fontFamily: "'NelnaLizzyChae', cursive", fontSize: 24, lineHeight: '28px', color: '#707072', alignSelf: 'stretch', zIndex: 1 }}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, margin: '0 20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '2px 4px 4px' }}>
          <Link href="/setup#tracker" style={{ fontFamily: f, fontSize: 12, fontWeight: 600, color: '#C7C7CC', textDecoration: 'none', letterSpacing: '.04em' }}>List →</Link>
        </div>
      </div>
    </div>
  );
            ...
            {user ? (
              <>
                <div style={{ fontFamily: f, fontSize: 13, color: '#9A9490', textAlign: 'center' as const }}>
                  라이브러리에서 Today ON으로 설정하면 여기에 표시됩니다
                </div>
                <Link href="/log?tab=라이브러리" style={{ fontFamily: f, fontSize: 13, fontWeight: 600, color: '#9A9490', textDecoration: 'none', marginTop: 4 }}>
                  라이브러리 보기 →
                </Link>
              </>
            ) : (
              <div style={{ fontFamily: f, fontSize: 13, color: '#9A9490', textAlign: 'center' as const }}>
                로그인 후 오늘의 룩과 스타일을 기록할 수 있어요
              </div>
            )}
          </div>
        )}
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ fontSize: 18, color: '#BCBAB6', cursor: 'grab', padding: '4px 6px', flexShrink: 0, touchAction: 'none', userSelect: 'none' as const }}
                    >
                      ☰
                    </div>
                  </div>
